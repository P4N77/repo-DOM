...
const fnControlaCaracteres = () =>{
    let tamTxtarea = document.getElementById('texto');
...
let btn3 = document.getElementById('btntext');
...
//ejercicio 4
let ingreso4= document.getElementById("Ingresotexto")
ingreso4.addEventListener("input",()=>{
    document.getElementById("entradaletras").innerHTML=ingreso4.value
})
...
const fnCaracteres = ()=>{
    let cantidadCar = document.getElementById("areakey").value.length;
    let disponibles = 50 -parseInt(cantidadCar);
    document.getElementById("contadorLetras").innerHTML = disponibles;
}

document.getElementById("areakey").addEventListener("keyup",fnCaracteres);
...
function fnEliminarElemento(evento) {
    if (evento.target.classList.contains("delete")){
        if (confirm("seguro eliminar elementos")) {
